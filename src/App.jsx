...
function Console({ appName, setAppName, onSignOut }) {
  ...
  useEffect(() => {
    ...
    const checkInquiryReminders = () => {
      ...
      if (due) {
        setActiveInquiryReminder(due);
        if (typeof Notification !== "undefined" && Notification.permission === "granted") {
          const notification = new Notification("AMS phone call reminder", {
            body: `Call ${due.customer} at ${due.phone} about the incomplete inquiry.`,
            tag: `ams-inquiry-call-${due.id}`,
    ...
    <div className="flex min-h-full w-full flex-col md:flex-row" style={{ background: PAPER, fontFamily: "'Inter', ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        ...
        <div className="px-4 sm:px-8 py-4 sm:py-6">
          ...
          {page === "inquiry-calls" && <InquiryCallsPage calls={inquiryCalls} setCalls={setInquiryCalls} phones={phones} onSaved={showToast} />}
...
function InquiryCallsPage({ calls, setCalls, phones, onSaved }) {
  const makeReminderInput = (baseValue, delayMinutes) => {
    const base = baseValue ? new Date(baseValue) : new Date();
    return localDateTimeInput(new Date((Number.isNaN(base.getTime()) ? new Date() : base).getTime() + Number(delayMinutes) * 60000));
  };
  const initialImportedAt = localDateTimeInput();
  const [form, setForm] = useState(() => ({ customer: "", phone: "", inquiry: "", importedAt: initialImportedAt, reminderDelay: "90", remindAt: makeReminderInput(initialImportedAt, 90), sendingPhoneId: "", remark: "" }));
  ...
  const activePhones = (phones || []).filter((item) => item.active !== false);
  const updateForm = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  const updateImportedAt = (value) => setForm((current) => ({ ...current, importedAt: value, remindAt: current.reminderDelay === "custom" ? current.remindAt : makeReminderInput(value, current.reminderDelay) }));
  const updateReminderDelay = (value) => setForm((current) => ({ ...current, reminderDelay: value, remindAt: value === "custom" ? current.remindAt : makeReminderInput(current.importedAt, value) }));
  const phoneLabel = (id) => {
    const selected = activePhones.find((item) => item.id === id) || (phones || []).find((item) => item.id === id);
    if (!selected) return "—";
    const name = selected.name || selected.phoneName || "Unnamed phone";
    const number = selected.number || selected.phoneNumber;
  ...
  const addCall = async (event) => {
    ...
    const record = { id: uid("INQ-"), customer: form.customer.trim(), phone: form.phone.trim(), inquiry: form.inquiry, importedAt: new Date(form.importedAt).toISOString(), remindAt: new Date(form.remindAt).toISOString(), sendingPhoneId: form.sendingPhoneId || null, remark: form.remark, status: "Pending", notifiedAt: null };
    try { await inquiryCallStore.create(record); setCalls((current) => [record, ...current]); const importedAt = localDateTimeInput(); setForm({ customer: "", phone: "", inquiry: "", importedAt, reminderDelay: "90", remindAt: makeReminderInput(importedAt, 90), sendingPhoneId: "", remark: "" }); onSaved("Inquiry call reminder saved."); }
  ...
  return <div className="flex flex-col gap-4">
    <div><h2 className="text-base font-semibold" style={{ color: INK }}>Inquiry Calls</h2><p className="text-xs" style={{ color: GRAY }}>Record an incomplete customer inquiry and receive a phone-call reminder after 1–2 hours.</p></div>
    <form onSubmit={addCall} className="rounded-xl border bg-white p-4 sm:p-5" style={{ borderColor: LINE }}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <label className="text-xs font-medium" style={{ color: GRAY }}>Customer<input value={form.customer} onChange={(event) => updateForm("customer", event.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm outline-none" style={{ borderColor: LINE }} placeholder="Customer name" /></label>
        <label className="text-xs font-medium" style={{ color: GRAY }}>Phone number<input value={form.phone} onChange={(event) => updateForm("phone", event.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm outline-none" style={{ borderColor: LINE }} placeholder="Phone number" /></label>
        <label className="text-xs font-medium" style={{ color: GRAY }}>Sending phone<select value={form.sendingPhoneId} onChange={(event) => updateForm("sendingPhoneId", event.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm outline-none bg-white" style={{ borderColor: LINE }}><option value="">Choose a sending phone</option>{activePhones.map((item) => <option key={item.id} value={item.id}>{phoneLabel(item.id)}</option>)}</select></label>
        <label className="text-xs font-medium" style={{ color: GRAY }}>Import date & time<input type="datetime-local" value={form.importedAt} onChange={(event) => updateImportedAt(event.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm outline-none" style={{ borderColor: LINE }} /></label>
        <label className="text-xs font-medium" style={{ color: GRAY }}>Reminder timing<select value={form.reminderDelay} onChange={(event) => updateReminderDelay(event.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm outline-none bg-white" style={{ borderColor: LINE }}><option value="30">After 30 minutes</option><option value="60">After 1 hour</option><option value="90">After 1.5 hours</option><option value="120">After 2 hours</option><option value="custom">Custom date & time</option></select></label>
        <label className="text-xs font-medium" style={{ color: GRAY }}>Call reminder date & time<input type="datetime-local" value={form.remindAt} onChange={(event) => setForm((current) => ({ ...current, remindAt: event.target.value, reminderDelay: "custom" }))} disabled={form.reminderDelay !== "custom"} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm outline-none disabled:bg-gray-100" style={{ borderColor: LINE }} /></label>
      </div>
      <label className="block text-xs font-medium mt-3" style={{ color: GRAY }}>Inquiry / missing information<textarea value={form.inquiry} onChange={(event) => updateForm("inquiry", event.target.value)} rows={2} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm outline-none" style={{ borderColor: LINE }} placeholder="What information is still missing?" /></label>
      <label className="block text-xs font-medium mt-3" style={{ color: GRAY }}>Remark<textarea value={form.remark} onChange={(event) => updateForm("remark", event.target.value)} rows={2} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm outline-none" style={{ borderColor: LINE }} placeholder="Optional internal remark" /></label>
      <div className="flex flex-wrap items-center gap-2 mt-4"><button type="submit" className="text-xs font-medium px-4 py-2.5 rounded-lg" style={{ background: INK, color: "white" }}><Plus size={13} className="inline mr-1" />Save inquiry call</button><button type="button" onClick={enableNotifications} className="text-xs font-medium px-4 py-2.5 rounded-lg border" style={{ borderColor: LINE, color: notificationState === "granted" ? GREEN : INK }}>{notificationState === "granted" ? "Notifications enabled" : "Enable laptop notification"}</button></div>
    </form>
    <section className="rounded-xl border bg-white overflow-auto" style={{ borderColor: LINE }}><div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: LINE }}><h3 className="text-sm font-semibold" style={{ color: INK }}>Call reminders</h3><span className="text-xs" style={{ color: GRAY }}>{activeCalls.length} pending</span></div>{activeCalls.length ? <table className="w-full min-w-[1000px] text-xs"><thead><tr className="text-left uppercase tracking-wide" style={{ color: "#9A9AA0" }}><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Phone</th><th className="px-4 py-3">Sending phone</th><th className="px-4 py-3">Import date/time</th><th className="px-4 py-3">Call reminder</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Action</th></tr></thead><tbody>{[...activeCalls].sort((a, b) => new Date(a.remindAt || 0) - new Date(b.remindAt || 0)).map((call) => <tr key={call.id} className="border-t" style={{ borderColor: LINE }}><td className="px-4 py-3 font-medium" style={{ color: INK }}>{call.customer}<div className="text-[11px] font-normal mt-1" style={{ color: GRAY }}>{call.inquiry || call.remark || "No extra details"}</div></td><td className="px-4 py-3" style={{ color: GRAY }}>{call.phone}</td><td className="px-4 py-3" style={{ color: GRAY }}>{phoneLabel(call.sendingPhoneId)}</td><td className="px-4 py-3 whitespace-nowrap" style={{ color: GRAY }}>{displayDateTime(call.importedAt)}</td><td className="px-4 py-3 whitespace-nowrap" style={{ color: call.status === "Pending" && new Date(call.remindAt) <= new Date() ? RED : GRAY }}>{displayDateTime(call.remindAt)}</td><td className="px-4 py-3" style={{ color: AMBER }}>{call.status}</td><td className="px-4 py-3 whitespace-nowrap"><div className="flex flex-wrap gap-2"><button type="button" onClick={() => markCalled(call)} className="rounded-lg px-2.5 py-1.5" style={{ background: GREEN_SOFT, color: GREEN }}>Mark called</button><button type="button" onClick={() => markCustomerResponded(call)} className="rounded-lg px-2.5 py-1.5" style={{ background: BLUE_SOFT, color: BLUE }}>Customer responded</button><button type="button" onClick={() => removeCall(call)} className="rounded-lg px-2.5 py-1.5" style={{ background: RED_SOFT, color: RED }}>Delete</button></div></td></tr>)}</tbody></table> : <p className="p-5 text-xs" style={{ color: GRAY }}>No active call reminders.</p>}</section>
    <section className="rounded-xl border bg-white overflow-hidden" style={{ borderColor: LINE }}><div className="px-5 py-4 flex items-center justify-between"><h3 className="text-sm font-semibold" style={{ color: INK }}>Reminder history</h3><button type="button" onClick={() => setShowHistory((value) => !value)} className="text-xs font-medium px-3 py-2 rounded-lg border" style={{ borderColor: LINE, color: INK }}>{showHistory ? "Hide history" : `View history (${historyCalls.length})`}</button></div>{showHistory && (historyCalls.length ? <div className="overflow-auto"><table className="w-full min-w-[950px] text-xs"><thead><tr className="text-left uppercase tracking-wide" style={{ color: "#9A9AA0" }}><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Phone</th><th className="px-4 py-3">Sending phone</th><th className="px-4 py-3">Import date/time</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Reminder time</th><th className="px-4 py-3">Action</th></tr></thead><tbody>{[...historyCalls].sort((a, b) => new Date(b.importedAt || 0) - new Date(a.importedAt || 0)).map((call) => <tr key={call.id} className="border-t" style={{ borderColor: LINE }}><td className="px-4 py-3 font-medium" style={{ color: INK }}>{call.customer}</td><td className="px-4 py-3" style={{ color: GRAY }}>{call.phone}</td><td className="px-4 py-3" style={{ color: GRAY }}>{phoneLabel(call.sendingPhoneId)}</td><td className="px-4 py-3 whitespace-nowrap" style={{ color: GRAY }}>{displayDateTime(call.importedAt)}</td><td className="px-4 py-3" style={{ color: GREEN }}>{call.status}</td><td className="px-4 py-3 whitespace-nowrap" style={{ color: GRAY }}>{displayDateTime(call.remindAt)}</td><td className="px-4 py-3 whitespace-nowrap"><button type="button" onClick={() => removeCall(call)} className="rounded-lg px-2.5 py-1.5" style={{ background: RED_SOFT, color: RED }}>Delete</button></td></tr>)}</tbody></table></div> : <p className="p-5 text-xs" style={{ color: GRAY }}>No completed inquiry calls yet.</p>)}</section>
  </div>;
}
